...
# Calculate percentage growth for each forecasted quarter
forecast_df['pct_growth'] = forecast_df['mean'].pct_change() * 100
...
# Annotate forecasted net profit values and percentage growth above the dots
for i, value in enumerate(forecast_df['mean']):
    ...
    # Display the percentage growth if available
    if i > 0:  # Skip the first entry as it doesn't have a prior period for growth calculation
        growth = forecast_df['pct_growth'].iloc[i]
        plt.text(forecast_df.index[i], value, f'({growth:.2f}%)', color='purple', fontsize=7, ha='left', va='top')
